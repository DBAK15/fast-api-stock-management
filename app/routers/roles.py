...
async def create_role(role: RoleCreate, db: Session = db_dependency):
...
async def real_all(db: Session = db_dependency):
    roles = db.query(Roles).all()
    return roles


@router.get("/role/{role_id}", response_model=RoleRead)
async def read_role(role_id: int, db: Session = db_dependency):
...
async def update_role(role_id: int, role_update: RoleUpdate, db: Session = db_dependency):
    role = db.query(Roles).filter(Roles.id == role_id).first()
    if not role:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Role not found")
...
async def delete_role(role_id: int, db: Session = db_dependency):
    role = db.query(Roles).filter(Roles.id == role_id).first()
    if not role:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Role not found")
